...
ASensorManager* AcquireASensorManagerInstance(struct android_app* app) {
  ...
  if (getInstanceForPackageFunc) {
    ...
    android_content_Context = (*env)->GetObjectClass(env, app->activity->clazz);
    midGetPackageName = (*env)->GetMethodID(env, android_content_Context,
                                                   "getPackageName",
                                                   "()Ljava/lang/String;");
    packageName= (jstring)((*env)->CallObjectMethod(env, app->activity->clazz, midGetPackageName));

    nativePackageName = (*env)->GetStringUTFChars(env, packageName, 0);
